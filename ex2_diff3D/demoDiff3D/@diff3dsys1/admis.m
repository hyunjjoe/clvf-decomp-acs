...
%% Apply computation based on the policy
  ...
  u_ad_min(ind_1) = uOpt{2}(ind_1);
  u_ad_max(ind_2) = uOpt{2}(ind_2);

  
  u_ad_max(ind_1) = max(min(((data(ind_1) - dataLast(ind_1))./deltaT - deriv{1}(ind_1).*dx{1}(ind_1).*deltaT)./(deriv{2}(ind_1).*deltaT), u(2)), u(1));
