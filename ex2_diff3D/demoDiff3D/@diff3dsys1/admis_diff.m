function [u_ad_min, u_ad_max] = admis_diff(obj, yLast, y, schemeData, conv, deltaT)
...
%% Apply computation based on the policy
  ...
  u_ad_min(ind_1) = uOpt{1}(ind_1);
  u_ad_max(ind_2) = uOpt{1}(ind_2);

  
  u_ad_max(ind_1) = max(min((-conv-(gamma*dataLast(ind_1)) - deriv{2}(ind_1).*dx{2}(ind_1).*deltaT)./(deriv{1}(ind_1).*deltaT), u(2)), u(1));
  u_ad_min(ind_2) = min(max((-conv-(gamma*dataLast(ind_2)) - deriv{2}(ind_2).*dx{2}(ind_2).*deltaT)./(deriv{1}(ind_2).*deltaT), u(1)), u(2));
  ...
  u_ad_max(u_ad_max<u_ad_min) = uOpt{2}(u_ad_max<u_ad_min);
  u_ad_min(u_ad_max<u_ad_min) = uOpt{2}(u_ad_max<u_ad_min);
