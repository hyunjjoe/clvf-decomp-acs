...
for i = 1:1: length(tau)-1
    V1_i = eval_u(sub.g,datas1(:,:,i) , [xs(:,1),xs(:,2)]);
    V1_i_p1 = eval_u(sub.g,datas1(:,:,i+1), [xs(:,1),xs(:,3)]);
    data1_i = reshape(V1_i,61,61,61);
    data1_i = data1_i(:,:,1);
    data1_i_p1 = reshape(V1_i_p1,61,61,61);
    data1_i_p1 = data1_i_p1(:,:,1);
    Der1 = computeGradients(sub.g,datas1(:,:,i));
    der1 = eval_u(sub.g,Der1{1},[xs(:,1),xs(:,2)]);
%     grad1 = reshape(der1,61,61,61);
    u(:,1) = (der1>=0)*(-1) + (der1<0)*(1);

    

    V2_i = eval_u(sub.g,datas2(:,:,i) , [xs(:,1),xs(:,2)]);
    V2_i_p1 = eval_u(sub.g,datas2(:,:,i+1), [xs(:,1),xs(:,3)]);
    data2_i = reshape(V2_i,61,61,61);
    data2_i = data2_i(:,:,1);
    data2_i_p1 = reshape(V2_i_p1,61,61,61);
    data2_i_p1 = data2_i_p1(:,:,1);
    Der2 = computeGradients(sub.g,datas2(:,:,i));
    der2 = eval_u(sub.g,Der2{1},[xs(:,1),xs(:,3)]);
    u(:,2) = (der2>=0)*(-1) + (der2<0)*(1);

    %% Compute subsys ACSs
    [u_ad_min1, u_ad_max1] = clvf_adms(data1_i, data1_i_p1, schemeData, 0.001, dt);
    [u_ad_min2, u_ad_max2] = clvf_adms(data2_i ,data2_i_p1, schemeData, 0.001, dt);
...
Vrec = recon.datag00;
level = min(eval_u(g, Vrec, xs1),[],'all');
